...
const HeaderLink = ({ children, ...props }: NavLinkProps) => {
  return (
    <NavLink
      ...
      {...props}
      class={`
      relative min-w-16 rounded rounded-b-none border border-slate-300 bg-slate-100 px-2 py-1
      text-center font-fancy text-slate-900 underline
    ...
    >
      ...
      <div
        class={`
        absolute -inset-x-2 -bottom-1 h-1 scale-x-0 rounded bg-black transition-transform
        ease-in-out

        group-hover:scale-x-25

        group-focus-visible:bg-blue-500
...
export default function Route({
  ...
  return (
    <div class={`flex flex-col`}>
      <header
        class={`
        grid w-full grid-cols-[1fr_auto_1fr] border-b border-black bg-slate-100 px-2 py-1
        shadow
        `}
      >
        <div class={`col-start-2 flex`}>
          ...
          <nav
            class={`mx-auto flex gap-4 rounded-4xl border-t border-slate-900 px-4 py-2`}
          >
            {links.map(({ to, label }) => {
              return (
